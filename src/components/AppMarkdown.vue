...
<script lang="ts" setup>import { emailRecipients, getEmail } from '@/globals';
import { ref } from 'vue';
...
const customHTMLContent = ref(html);
renderEmail();
...
function renderEmail() {
  for (const name of emailRecipients) {
    ...
    customHTMLContent.value = customHTMLContent.value.replaceAll(`@${name}@`, emailLink);
  }
}
</script>
