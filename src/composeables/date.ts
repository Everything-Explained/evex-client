...
export function useDate(date: Date | ISODateStr | MsDateFormat) {
  ...
  return {
    ...
    toHybridTime() {
      ...
      if (secondsSinceThen >= timeSpan.year || relativeTime.includes('year')) {
    ...
    toRelativeTime() {
      if (!Intl || !Intl.RelativeTimeFormat) return this.toShortDate(); // Support old iOS version
...
function to12Hours(hours: number) {
  return hours > 12 ? hours - 12 : hours;
}
function padTime(time: number) {
  return time < 10 ? `0${time}` : `${time}`;
}
function getAMPM(hours: number) {
