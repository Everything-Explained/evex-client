...
const api = useAPI();
const { isPending } = api;
const { isValidated, validate } = useInputValidation(1);
    ...
    <transition name="fade" mode="out-in">
      <div v-if="!loginSuccess" class="red-login">
        <ux-text type="block" custom-class="red-login__disclaimer">
          This page contains sensitive content which
          <em>requires authentication</em>. If you have a passcode, use the form
          below to grant yourself access. <br /><br />
        ...
        <ux-text type="span-block" custom-class="red-login__disclaimer">
          <strong>CAVEAT:</strong> The passcode will only be saved for
          <em>this device</em>. In order to view this content on your other
          devices:
      ...
      <div v-else>
        <ux-text type="block">
          You have <b>successfully</b> gained access to the
          <em>RED33M</em> section. <br /><br />
          If you open the menu, you will notice that under the RED33M header,
          there will now be two new options: <b>Literature</b> and
          <b>Videos</b>. <br /><br />
          Unlike the public Literature page, there are no categories, since they
          fall under the <em>RED33M</em> category. If you have any general
          questions or concerns you can goto our
          <router-link to="/support">Support Page</router-link>. However if you
          need specific help with something that our Support page does not
          cover, feel free to email <b>Ethan</b> for any
          <em>one on one</em> needs.
        </ux-text>
        <page-footer />
      </div>
    </transition>
